...
function get_siteid() {
	...
	if (defined('IN_ADMIN')) {
		if (($d = param::get_cookie('siteid'))!=FALSE) {
			$siteid = $d;
		} else {
			return '';
		}
		...
		if(in_array($method,$legalmethods)) {
			foreach (get_declared_classes() as $class){
			   ...
			   if($refclass->isSubclassOf('hook')){
				  if (($_method = $refclass->getMethod($method))!=FALSE) {
...
function cache_page_start() {
	...
	$relate_url = isset($_SERVER['REQUEST_URI']) ? safe_replace($_SERVER['REQUEST_URI']) : $_SERVER['PHP_SELF'].(isset($_SERVER['QUERY_STRING']) ? '?'.safe_replace($_SERVER['QUERY_STRING']) : $_SERVER['PATH_INFO']);
