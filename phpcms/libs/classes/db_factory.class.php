...
final class db_factory {
	 ...
	 * @return db_factory
	 */
	public function get_database($dbconfig) {
	  
		if(!isset($this->db_list[$dbconfig]) || !is_object($this->db_list[$dbconfig])) {
			$this->db_list[$dbconfig] = $this->connect($dbconfig);
		}
		return $this->db_list[$dbconfig];
	}
...
// 				break;
			case 'mysqlx' :
				pc_base::load_sys_class('mysqlx','',0);
				$object=new mysqlx();
				break;
			case 'access' :
